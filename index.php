...
ArrestDB::Serve('GET', function () {
    ...
    if (isset($_GET['function']) === true) {
        ...
        if ($function == 'list') {
            $query = array
            (
                sprintf('SELECT * FROM inbounds'),
            );
            if (isset($_GET['id']) === true) {
                $query[] = sprintf('WHERE id = %u', $_GET['id']);
            }
...
class ArrestDB
{
        ...
        try {
            if (isset($db, $query) === true) {
                ...
                (
                    ...
                    \PDO::SQLITE_OPEN_READONLY => false
                );
                ...
                $db = new \PDO($query, null, null, $options);
                $pragmas = array
                (
                    'automatic_index' => 'ON',
                    'cache_size' => '8192',
    ...
    public static function Serve($on = null, $callback = null)
    {
        ...
        if ((empty($on) === true) || (strcasecmp($_SERVER['REQUEST_METHOD'], $on) === 0)) {
            ...
            return (empty($callback) === true) ? true : exit(call_user_func($callback));
        }
